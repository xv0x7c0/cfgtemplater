...
def first_address(addr):
    """ Return the first address of a subnet, or self in case of hostaddress is given"""
    ip = ipaddress.ip_interface(addr)
    prefixlen = ip.network.prefixlen
    version = re.sub('Interface', '', ip.__class__.__name__)
    if (version == 'IPv4' and prefixlen == 32) or (version == 'IPv6' and prefixlen == 128):
        return str(ip)
    return f"{next(ip.network.hosts())}/{prefixlen}"


def last_address(addr):
    """ Return the last address of a subnet, or self in case of hostaddress is given"""
    ip = ipaddress.ip_interface(addr)
    prefixlen = ip.network.prefixlen
    version = re.sub('Interface', '', ip.__class__.__name__)
    if (version == 'IPv4' and prefixlen == 32) or (version == 'IPv6' and prefixlen == 128):
...
def uncompress(addr):
    ...
    ip = ipaddress.ip_interface(addr)
    version = re.sub('Interface', '', ip.__class__.__name__)
    if version == 'IPv4': return '.'.join(list(map(lambda x: '%03d' % int(x), str(ip.ip).split('.'))))
    if version == 'IPv6': return ip.ip.exploded

def octets(addr):
    """ Return a list of octets"""
    ip = ipaddress.IPv4Interface(addr).ip
    return list(map(lambda x: int(x), str(ip).split('.')))
