...
def test_is_ip_net():
    ...
    assert ip_tests.is_ip_net('2001:db8::1/128') ==True
    assert ip_tests.is_ip_net('2001:db8::/127') == True
    assert ip_tests.is_ip_net('blah') == False

def test_is_ipv4_net():
    assert ip_tests.is_ipv4_net('192.0.2.0/24') == True
    assert ip_tests.is_ipv4_net('192.0.2.1/24') == False
    assert ip_tests.is_ipv4_net('2001:db8::1/128') == False
...
def test_is_ipv6_net():
    ...
    assert ip_tests.is_ipv6_net('2001:db8::/127') == True
    assert ip_tests.is_ipv6_net('2001:db8::1/127') == False
